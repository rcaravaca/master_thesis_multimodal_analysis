...
def baselines_excecution(sex):
	...
	param_grid = {'C': [0.001, 0.1, 1, 10, 100, 200],
	...
	for target in targets:
		...
		grid_v = GridSearchCV(svm.SVC(probability=True),param_grid,refit=True,verbose=0, scoring="roc_auc", cv=3)
		grid_w = GridSearchCV(svm.SVC(probability=True),param_grid,refit=True,verbose=0, scoring="roc_auc", cv=3)
		...
		for o in range(30):
			...
			mean_auc_v, std_auc_v, predict_probas_v, decision_function_v, conf_matrix_v, conf_matrix_v_std, predictions_v = bu.get_roc_curve(svm_video, Xv, Yv, target, trains, tests, "video"+sex)
			bu.img_confusion_matrix(conf_matrix_v, conf_matrix_v_std, [0, 1], title="Video classifier for "+str(target)+" target")

			mean_auc_w, std_auc_w, predict_probas_w, decision_function_w, conf_matrix_w, conf_matrix_w_std, predictions_w = bu.get_roc_curve(wearable_model, Xw, Yw, target, trains, tests, "wearables"+sex)
			bu.img_confusion_matrix(conf_matrix_w, conf_matrix_w_std, [0, 1], title="Wearables classifier for "+str(target)+" target")

			mean_auc_mv, std_auc_mv, prob, decision_function_mv, conf_matrix_mv, conf_matrix_mv_std = bu.get_roc_curve_multimodal(majority_vote2, [Xv, Xw], [Yv, Yw], target, trains, tests, "Majority_vote"+sex)
...
if __name__ == "__main__":
	...
	for sex in ["_all", "_F", "_M"]:
	# for sex in ["_F"]:
		baselines_excecution(sex)
		os.system("mv *pdf sync_only_* auc_sync_only_* NEW_VIDEO_BL_ONE_DAY_OUT_EXPERIMENTS/sync_only"+sex)
