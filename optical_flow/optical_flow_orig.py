...
def get_parser():
    ...
    parser.add_argument("-s","--scale", help="Scale the video.", default=1, type=float)
...
if __name__ == "__main__":
    ...
    if args.video:
        ...
        H_DIM = int(args.scale*height)
        W_DIM = int(args.scale*width)
        ...
        hsv = np.zeros((W_DIM,H_DIM,3), dtype=np.uint8)
        hsv[:,:,1] = 255
        ...
        while(video.isOpened()):
            ...
            if ret:
                ...
                frame = cv2.resize(frame_bgr, (W_DIM,H_DIM), interpolation = cv2.INTER_AREA)
