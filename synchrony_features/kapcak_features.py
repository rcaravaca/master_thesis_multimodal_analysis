...
def get_pds(signal, fs = 20):
	
	f, Pxx_spec = sg.welch(signal, fs, nperseg=len(signal))
	# print(signal)
	# print(Pxx_spec)
...
def get_complex_features(signal_a,signal_b,low_features_a,low_features_b, only_synchrony=False):
	...
	if not only_synchrony:
		...
		all_features = [person_corr,mutual_info,mimicry_feat,lagged_correlation]


	return all_features

def get_low_level_features(signal, w, fs=20):

	low_features = []
	for i in np.arange(0,len(signal)-w*fs//2,(w*fs)//2):
		pds = get_pds(signal[i:i+w*fs-1], fs)
		low_features.append([signal[i:i+w*fs-1].mean(), signal[i:i+w*fs-1].std(), pds[0],pds[1],pds[2],pds[3],pds[4],pds[5]])
